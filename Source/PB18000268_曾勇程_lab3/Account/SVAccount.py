...
from Account.AccountUI.SVAccountInf import Ui_SVAccount
from Account.SVAccUpdate import SVAccUpdate
from db import *
import re
...
class SVAccount(QDialog):
    "A dialog class for Ui_LoginDialog, who can show itself"

    def __init__(self, parent):
        super(SVAccount, self).__init__(parent)
        self.tabName_SV = 'SV_account'
        self.tabName = 'Transact'
        self.accountName = 'Account'
        self.ui = Ui_SVAccount()
        self.ui.setupUi(self)
        self.ui.table.setColumnCount(9)  # 不设置不显示这些列
        self.ui.table.setHorizontalHeaderLabels(['ID', 'balance', 'TIME_open', 'TIME_access', 'rate', 'currency', '支行名字', '所属客户身份证', '账户类型'])
    ...
    def Add(self):  # 增
        ...
        rate = self.ui.lineEdit_Rate.text()
        currency = self.ui.lineEdit_Cu.text()
        ...
        # 首先进行错误检查
        if ID == '' or balance == '' or time_open == '' or TIME_access == '' or rate == '' or currency == '' or name == '' or ID_card == '':
        ...
        currency = StringPre(currency)  # 预处理，应对特殊字符

        # 插入 SV_Account的记录
        self.query = 'insert into ' + self.tabName_SV + ' value(' + IsString(ID, False) + ', ' + IsString(balance, False) + ', ' + \
                      IsString(time_open) + ', ' + IsString(TIME_access) + ', ' + IsString(rate, False) + ', ' \
                     + IsString(currency) + ');'

        # 插入 Account的记录
        self.quAcc = 'insert into ' + self.accountName + ' value(' + IsString(ID, False) + ', ' + IsString(balance,False) + ', ' + \
                     IsString(time_open) + ', ' + IsString(TIME_access) + ');'

        self.query_tran = 'insert into ' + self.tabName + ' value(' + IsString(name) + ', ' + \
    ...
    def Delete(self):  # 增
        ...
        # 销毁 SV_Account的记录
        if rate != '':
            self.query = 'delete from ' + self.tabName_SV + ' where ' + ISEmpty('ID', ID, False) + ' and ' \
                         + ISEmpty('balance', balance, False) + ' and ' + ISEmpty('TIME_open', time_open)+ \
                         ' and ' + ISEmpty('TIME_access', TIME_access) + ' and ' + 'rate LIKE ' + rate  \
                         + ' and ' + ISEmpty('currency', currency) + ';'
        else:
            self.query = 'delete from ' + self.tabName_SV + ' where ' + ISEmpty('ID', ID, False) + ' and ' \
                         + ISEmpty('balance', balance, False) + ' and ' + ISEmpty('TIME_open', time_open) + \
    ...
    def Check(self):
        ...
        if rate == '':
            self.query = 'select * from ' + self.tabName_SV + ', ' + self.tabName + ' where Transact.ID = SV_account.ID and '\
                         + ISEmpty('SV_account.ID', ID, False) + ' and ' \
                         + ISEmpty('balance', balance, False) + ' and ' + ISEmpty('TIME_open', time_open) + \
                         ' and ' + ISEmpty('TIME_access', TIME_access) + ' and ' + ISEmpty('currency', currency) \
                         + ' and '+ ISEmpty('name', name) + ' and ' +\
            ...
            self.query = 'select * from ' + self.tabName_SV + ', ' + self.tabName + ' where Transact.ID = SV_account.ID and '\
                         ...
                         + ISEmpty('balance', balance, False) + ' and ' + ISEmpty('TIME_open', time_open) + \
                         ' and ' + ISEmpty('TIME_access', TIME_access) + ' and ' + 'rate LIKE ' + rate  \
                         + ' and ' + ISEmpty('currency', currency) + ' and '+ ISEmpty('name', name) + ' and ' +\
                     ISEmpty('ID_card', ID_card) + ' and ' + ISEmpty('Type', acctype, False) + ';'
        print(self.query)
    ...
    def CheckShow(self, Items):
        ...
        for record in Items:
            ...
            rate = QTableWidgetItem(str(record[4]))
            rate.setTextAlignment(QtCore.Qt.AlignCenter)
            currency = QTableWidgetItem(str(record[5]))
            currency.setTextAlignment(QtCore.Qt.AlignCenter)
            #######################
            name = QTableWidgetItem(str(record[6]))
            name.setTextAlignment(QtCore.Qt.AlignCenter)
            ID_card = QTableWidgetItem(str(record[7]))
            ID_card.setTextAlignment(QtCore.Qt.AlignCenter)
            acctype = QTableWidgetItem(str(record[8]))
            ...
            self.ui.table.setItem(currentRowCount, 4, rate)  # 列5
            self.ui.table.setItem(currentRowCount, 5, currency)  # 列6
            self.ui.table.setItem(currentRowCount, 6, name)  # 列7
            self.ui.table.setItem(currentRowCount, 7, ID_card)  # 列8
            self.ui.table.setItem(currentRowCount, 8, acctype)  # 列9
            currentRowCount += 1
            self.ui.table.setRowCount(currentRowCount)

    def renderTable(self):
        self.ui.table.setRowCount(0)
        query = 'select * from SV_account, Transact where Transact.ID = SV_account.ID;'
