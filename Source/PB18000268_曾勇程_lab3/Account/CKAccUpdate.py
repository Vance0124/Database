...
from Account.AccountUI.CKAccountUpdate import Ui_CKAccUpdate
...
class CKAccUpdate(QDialog):
    "A dialog class for Ui_LoginDialog, who can show itself"

    def __init__(self, parent):
        super(CKAccUpdate, self).__init__(parent)
        self.ui = Ui_CKAccUpdate()
        self.ui.setupUi(self)
        self.before = ''    # 用于修改CK_Account
        self.accbefore = ''   # 用于修改Account
        self.query = ''     # 用于修改CK_Account
    ...
    def Update(self):
        ...
        overdraft_be = self.ui.Before_4.text()
        ...
        overdraft_af = self.ui.After_4.text()
        ...
        self.before = ISEmpty('ID', ID_be, False) + ' and ' + ISEmpty('balance', balance_be, False) + ' and ' \
                      ...
                      ISEmpty('overdraft', overdraft_be, False) +';'
        ...
        if overdraft_af != '':
            NotEmpty_af.append(('overdraft', overdraft_af))

        Num_af = len(NotEmpty_af)
        Num_af_Flag = 1     # 标记是否需要修改 CK表
        if Num_af == 0:
            Num_af_Flag = 0
        ...
        self.query = 'update ' + self.tabName_CK + ' set ' + Str_af + ' where ' + self.before
###################################################################################################################
        acctype = '0'   # 这里开始修改 Transact
